...
class KGModel(nn.Module, ABC):
    ...
    def __init__(self, sizes, rank, dropout, gamma, data_type, bias, init_size, cpu):
        """Initialize KGModel."""
        ...
        self.bh.weight.requires_grad = (self.bias == "learn")
        self.bt.weight.requires_grad = (self.bias == "learn")
    ...
    def get_rhs(self, queries, eval_mode):
        ...
        Returns:
             rhs_e: torch.Tensor with targets' embeddings
                    if eval_mode=False returns embedding of tail entities (B x rank)
                    else returns embedding of all possible entities in the KG dataset (B x rank)
             rhs_biases: torch.Tensor with targets' biases
                         if eval_mode=False returns biases of tail entities (B x 1)
                         else returns biases of all possible entities in the KG dataset (B x 1)
        """
        pass
    ...
    def score(self, lhs, rhs, eval_mode):
        ...
        Returns:
            score: torch.Tensor with scores of queries against targets
                   if eval_mode=True, returns scores against all possible tail entities, shape (B x n_entities)
                   else returns scores for triples in batch shape (B x 1)
        """
        ...
        if self.bias == 'constant':
            ...
            if eval_mode:
                ...
                return torch.abs(lhs_biases - rhs_biases.t()) + score
            else:
                # return lhs_biases + rhs_biases + score
                return torch.abs(lhs_biases - rhs_biases) + score
        else:
            return score
    ...
    def forward(self, queries, eval_mode=False):
        ...
        Returns:
            predictions: torch.Tensor with triples' scores
                         shape: (batch x 1) if eval_mode is false, else (batch x n_entities)
        ...
        # get factors for regularizationf
    ...
    def get_ranking(self, queries, filters, batch_size=1000):
        ...
        with torch.no_grad():
            ...
            while b_begin < len(queries):
                ...
                if not self.CPU: 
                    these_queries = these_queries.cuda()
