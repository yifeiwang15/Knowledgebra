...
class KGOptimizer(object):
    ...
    def neg_sampling_loss(self, input_batch):
        ...
        negative_score = (F.softmax(negative_score * self.adversarial_temperature, dim = 0).detach() \
                          * F.logsigmoid(-negative_score)).sum(dim = 0) * self.neg_sample_size

        # loss = - torch.cat([positive_score, negative_score], dim=0).mean()
        loss = - (positive_score.mean() +  negative_score.mean()) / (self.neg_sample_size + 1)

        # print((positive_score.mean()/negative_score.mean()).item())
        return loss, factors
    ...
    def epoch(self, examples):
        ...
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            counter = 0
            while b_begin < examples.shape[0]:
                # print("Entity embedding weights range: ({:12f}, {:12f})".format(
                #         self.model.entity.weight.data.min().item(),
                #         self.model.entity.weight.data.max().item()
                #     )
                # )
