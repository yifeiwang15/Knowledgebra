...
class KGOptimizer(object):
    ...
    def __init__(
        ...
        self.optimizer_ent = optimizer_ent
        self.optimizer_rel = optimizer_rel
        self.batch_size = batch_size
        self.neg_sample_size = neg_sample_size
        self.pn_loss_ratio = pn_loss_ratio
    ...
    def reduce_lr(self, factor=0.5623413251903491):
        """Reduce learning rate.

        Args:
            factor: float for the learning rate decay
        """
        for param_group in self.optimizer_ent.param_groups:
            param_group['lr'] *= factor
        for param_group in self.optimizer_rel.param_groups:
            param_group['lr'] *= factor
    ...
    def neg_sampling_loss(self, input_batch):
        ...
        negative_score = (F.softmax(negative_score * self.adversarial_temperature, dim = 0).detach() \
                          * F.logsigmoid(-negative_score)).sum(dim = 0) * self.pn_loss_ratio

        # loss = - torch.cat([positive_score, negative_score], dim=0).mean()
        loss = - (positive_score.mean() +  negative_score.mean()) / (self.pn_loss_ratio + 1)
    ...
    def calculate_valid_loss(self, examples):
        ...
        with torch.no_grad():
            while b_begin < examples.shape[0]:
                input_batch = examples[b_begin:b_begin + self.batch_size]
                if not self.CPU: 
